# 契約審査・契約運用の後工程 / 03_specification
    ...
    ## 2. 境界づけられたコンテキスト
    - **Contract Intake**: 契約要求の起票
- **Negotiation Control**: 差分・逸脱・承認
- **Execution**: 締結と版管理
- **Obligation Management**: 通知・報告・支払・監査協力などの義務
- **Renewal & Amendment**: 更新・解約・変更
- **Compliance & Audit**: 規程準拠と証跡
...
ContractRequest "1" --> "0..1" Contract : becomes
Contract "1" --> "*" Clause : contains
Clause "1" --> "0..*" Deviation : gives_rise_to
Contract "1" --> "*" Obligation : governs
Contract "1" --> "0..*" RenewalCase : reviewed_by
Contract "1" --> "*" AuditRecord : emits
    ```
